...
localparam
    ...
    ADDR_W_DATA_0        = 7'h30,
    ADDR_W_CTRL          = 7'h34,
    ADDR_W_KERNEL_DATA_0 = 7'h38,
    ADDR_W_KERNEL_DATA_1 = 7'h3c,
    ADDR_W_KERNEL_CTRL   = 7'h40,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_W_KERNEL_DATA_0: begin
                    rdata <= int_w_kernel[31:0];
                end
                ADDR_W_KERNEL_DATA_1: begin
                    rdata <= int_w_kernel[63:32];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_W_KERNEL_DATA_0)
            int_w_kernel[31:0] <= (WDATA[31:0] & wmask) | (int_w_kernel[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_w_kernel[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_W_KERNEL_DATA_1)
            int_w_kernel[63:32] <= (WDATA[31:0] & wmask) | (int_w_kernel[63:32] & ~wmask);
    end
end
