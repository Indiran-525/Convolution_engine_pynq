...
architecture behave of convbas_CTRL_s_axi is
    ...
    constant ADDR_H_CTRL          : INTEGER := 16#2c#;
    constant ADDR_W_DATA_0        : INTEGER := 16#30#;
    constant ADDR_W_CTRL          : INTEGER := 16#34#;
    constant ADDR_W_KERNEL_DATA_0 : INTEGER := 16#38#;
    constant ADDR_W_KERNEL_DATA_1 : INTEGER := 16#3c#;
    constant ADDR_W_KERNEL_CTRL   : INTEGER := 16#40#;
    ...
    signal int_w_kernel        : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_W_KERNEL_DATA_0 =>
                        rdata_data <= RESIZE(int_w_kernel(31 downto 0), 32);
                    when ADDR_W_KERNEL_DATA_1 =>
                        rdata_data <= RESIZE(int_w_kernel(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    w_kernel             <= STD_LOGIC_VECTOR(int_w_kernel);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_w_kernel(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_W_KERNEL_DATA_0) then
                    int_w_kernel(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_w_kernel(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_w_kernel(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_W_KERNEL_DATA_1) then
                    int_w_kernel(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_w_kernel(63 downto 32));
                end if;
            end if;
        end if;
    end process;
